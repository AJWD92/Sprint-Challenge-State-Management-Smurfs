...
- [x] What problem does the context API help solve?
      It solve the propblem of prop drilling at the application level state.
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
      State is a read-only property so the only way to change state is to use an action a.k.a an object describing what happened, the changes are made with pure functions know as reducers which specify how the state tree is transformed by actions, and the store is the single source of truth where your state tree is stored
- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is stored in th store using redux when multipule components will need access to the state tree where a component state level is not accessible from other components  unless it is explicitly passed as props to it's sub somponents.
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
      it lets you write action creators that return a function instead of an action i.e using dispatch
- [x] What is your favorite state management system you've learned and this sprint? Please explain why!
I think if I had more exposure to context api I would like that more as it seems easier to pass the state tree at the app level around a little easier
